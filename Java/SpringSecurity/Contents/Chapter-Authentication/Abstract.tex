...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\small]
    \node(browser) [block,drop shadow, fill=blue!30] at (0,0) {Client(Browser)};
    \node(filter1) [block,drop shadow, fill=orange!20] at (0,-1.5) {Filter};
    \node(proxy) [block,drop shadow, fill=orange!20] at (0,-3) {DelegatingFilterProxy \\ \footnotesize{(FilterChainProxy)}};
    \node(filter2) [block,drop shadow, fill=orange!20] at (0,-4.5) {Filter};
    \node(servlet) [block,drop shadow, fill=blue!30] at (0,-6) {Servlet};
